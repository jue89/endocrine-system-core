...
class EndocrineSystem extends events.EventEmitter {
		...
		server.on( 'ready', () => {
			...
			server.authorizePublish = ( client, topic, payload, callback ) => {
				this._decide( this._acIn, client, topic, ( err, decision ) => {
					...
					// Store stats
					if( this._stats ) {
						this._stats.bytesIn += env.pkgLength;
			...
			server.authorizeForward = ( client, packet, callback ) => {
				this._decide( this._acOut, client, packet.topic, ( err, decision ) => {
					...
					// Store stats
					if( this._stats ) {
						this._stats.bytesOut += env.pkgLength;
						this._stats.messagesOut += 1;
					}

					// Emit event
					this.emit( ( decision === true ) ? 'outPassed' : 'outRejected', env );
			...
			pem.getFingerprint( fs.readFileSync( options.caPath ) ).then( ( fingerprint ) => {
				...
				if( options.stats > 0 ) this._handleStats( options.stats * 1000 );
	...
	_handleStats( secs ) {
		...
		// If stats has been collected, emit the result
		if( this._stats ) this.emit( 'stats', this._stats );
		...
		// Start timeout
		this._statsTimeout = setTimeout( () => this._handleStats( secs ), secs );
